  ...
  class Graph
    ...
    def draw
      ...
      data_object.each do |data|
        ...
        end_date = agg.capture_max if agg.capture_max > end_date
      end
    ...
    def draw_date(graph_date, date_template)
      ...
      @canvas.text(position_x.to_i, position_y.to_i, graph_date.strftime("%F %l:%M")).styles(:fill=>"black", :font_size=>size)
    end
  end
end